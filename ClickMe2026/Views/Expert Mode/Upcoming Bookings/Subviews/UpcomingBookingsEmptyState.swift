                ...
                Button(action: onUpdateAvailability) {
                    Text("Update Availability")
                        ...
                        .background(Capsule().fill(UpcomingBookingsTheme.brandGreen)
                            .shadow(color: UpcomingBookingsTheme.brandGreen.opacity(0.50), radius: 20, x: 0, y: 6))
                }
    ...
    private var calendarEmptyIcon: some View {
        Canvas { ctx, size in
            ...
            ctx.fill(
                ...
                with: .color(UpcomingBookingsTheme.brandGreen.opacity(0.06))
            )

            // Dark circle
            let inner = CGRect(x: cx - r * 0.88, y: cy - r * 0.88, width: r * 1.76, height: r * 1.76)
            ctx.fill(Path(ellipseIn: inner), with: .color(UpcomingBookingsTheme.emptyIconBg))
            ctx.stroke(Path(ellipseIn: inner), with: .color(UpcomingBookingsTheme.emptyBorder), lineWidth: 1.5)
            ...
            ctx.stroke(Path(roundedRect: calRect, cornerRadius: 8), with: .color(UpcomingBookingsTheme.brandGreen), lineWidth: 2.5)

            // Top header bar
            let barRect = CGRect(x: cx0, y: cy0, width: cw, height: ch * 0.28)
            ctx.fill(Path(roundedRect: barRect, cornerRadius: 8), with: .color(UpcomingBookingsTheme.brandGreen.opacity(0.28)))
            ctx.stroke(Path(roundedRect: barRect, cornerRadius: 8), with: .color(UpcomingBookingsTheme.brandGreen), lineWidth: 2.5)

            // Tab notches
            for dx: CGFloat in [-cw * 0.22, cw * 0.22] {
                let tab = CGRect(x: cx + dx - 4, y: cy0 - ch * 0.13, width: 8, height: ch * 0.17)
                ctx.fill(Path(roundedRect: tab, cornerRadius: 3), with: .color(UpcomingBookingsTheme.brandGreen))
            }
            ...
            xPath.move(to: CGPoint(x: cx + xS, y: xCy - xS))
            xPath.addLine(to: CGPoint(x: cx - xS, y: xCy + xS))
            ctx.stroke(xPath, with: .color(UpcomingBookingsTheme.brandGreen), style: StrokeStyle(lineWidth: 3, lineCap: .round))
        }

...
private enum BookingSummaryPreviewRoute: Hashable {
    case fiveStars
    case noReviewYet
}

/// Wraps the session-summary screen inside a NavigationStack with a dummy
/// "My Bookings" parent already pushed, so the system back chevron renders
/// in the canvas.
private struct BookingSummaryPreviewHarness: View {
    let route: BookingSummaryPreviewRoute
    @State private var path: [BookingSummaryPreviewRoute]

    init(route: BookingSummaryPreviewRoute) {
        self.route = route
    ...
    var body: some View {
        NavigationStack(path: $path) {
            List {
                Text("Past sessions")
                NavigationLink("Session summary", value: route)
            }
            .navigationTitle("My Bookings")
            .navigationDestination(for: BookingSummaryPreviewRoute.self) { dest in
                switch dest {
                case .fiveStars:
...
#Preview("Session Summary — 5 Stars") {
    BookingSummaryPreviewHarness(route: .fiveStars)
        .preferredColorScheme(.dark)
}

#Preview("Session Summary — No Review Yet") {
    BookingSummaryPreviewHarness(route: .noReviewYet)
        .preferredColorScheme(.dark)
}

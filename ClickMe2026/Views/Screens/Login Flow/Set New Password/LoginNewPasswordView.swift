...
private enum NewPasswordPreviewRoute: Hashable {
    case `default`
    case weak
    case fair
    case strong
...
/// Wraps the set-new-password view inside a NavigationStack with a dummy
/// "Forgot password" parent already pushed, so the system back chevron
/// renders in the canvas.
private struct NewPasswordPreviewHarness: View {
    let route: NewPasswordPreviewRoute
    @State private var path: [NewPasswordPreviewRoute]

    init(route: NewPasswordPreviewRoute) {
        self.route = route
    ...
    var body: some View {
        NavigationStack(path: $path) {
            List {
                ...
                NavigationLink("Set new password", value: route)
            }
            .navigationTitle("Reset password")
            .navigationDestination(for: NewPasswordPreviewRoute.self) { dest in
                switch dest {
                case .default:
...
#Preview("Default") {
    NewPasswordPreviewHarness(route: .default)
        .preferredColorScheme(.dark)
}

#Preview("Weak Password") {
    NewPasswordPreviewHarness(route: .weak)
        .preferredColorScheme(.dark)
}

#Preview("Fair Password") {
    NewPasswordPreviewHarness(route: .fair)
        .preferredColorScheme(.dark)
}

#Preview("Strong Password") {
    NewPasswordPreviewHarness(route: .strong)
        .preferredColorScheme(.dark)
}

#Preview("Both Errors") {
    NewPasswordPreviewHarness(route: .bothErrors)
        .preferredColorScheme(.dark)
}

#Preview("Bad Code") {
    NewPasswordPreviewHarness(route: .badCode)
        .preferredColorScheme(.dark)
}

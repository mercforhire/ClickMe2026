...
private enum ModeSwitchPreviewRoute: Hashable {
    case modeSwitcher
}

/// Wraps the mode-switcher screen inside a NavigationStack with a dummy
/// "Settings" parent already pushed, so the system back chevron renders
/// in the canvas.
private struct ModeSwitchPreviewHarness: View {
    let route: ModeSwitchPreviewRoute
    @State private var path: [ModeSwitchPreviewRoute]

    init(route: ModeSwitchPreviewRoute) {
        self.route = route
    ...
    var body: some View {
        NavigationStack(path: $path) {
            List {
                Text("Account")
                NavigationLink("Mode Switcher", value: route)
            }
            .navigationTitle("Settings")
            .navigationDestination(for: ModeSwitchPreviewRoute.self) { _ in
                ModeSwitchView()
            }
        }
    }
...
#Preview("Client Active") {
    ModeSwitchPreviewHarness(route: .modeSwitcher)
